...
function saveStateToUrl(scores: Record<Dimension, number>, resultCode: string) {
  const params = new URLSearchParams({
    o: String(scores.order),
    e: String(scores.energy),
    p: String(scores.pace),
    m: String(scores.emotion),
    r: resultCode,
  });
...
function loadStateFromUrl(): { scores: Record<Dimension, number>; resultCode: string } | null {
  ...
  // Support abbreviated params: o=order, e=energy, p=pace, m=emotion, r=result
  if (!params.has('o')) return null;
  return {
    scores: {
      order: parseInt(params.get('o') || '0'),
      energy: parseInt(params.get('e') || '0'),
      pace: parseInt(params.get('p') || '0'),
      emotion: parseInt(params.get('m') || '0'),
    },
    resultCode: params.get('r') || '',
  };
}
